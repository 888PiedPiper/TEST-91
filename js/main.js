// ==================== НАСТРОЙКИ ====================
const SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbzWdlqcf4Zw91E6Av5IvtZx_8LQasuDKTYyKvseNV4swbkZuW8UNM1Oj3QA8wMjdTlS/exec';
...
const translations = {
    ru: {
        ...
        'draw_status': 'Распределение по группам:',
        'group_a': 'Группа A:',
        'group_b': 'Группа B:',
        'teams_left': 'Осталось команд:',
        'draw_completed': 'Жеребьёвка завершена! Нажмите "Сохранить жеребьёвку".',
        'draw_waiting': 'пока пусто',
        ...
        // ДОБАВЛЕННЫЕ ПЕРЕВОДЫ (статусы и сообщения)
        ...
        // Админ-панель (лейблы в карточках матчей)
        'admin_score': 'СЧЁТ',
        'admin_points': 'ОЧКИ',
        'admin_date': 'ДАТА',
        'admin_link': 'LINK',
    ...
    en: {
        ...
        'draw_status': 'Group distribution:',
        'group_a': 'Group A:',
        'group_b': 'Group B:',
        'teams_left': 'Teams left:',
        'draw_completed': 'Draw completed! Click "Save draw".',
        'draw_waiting': 'empty',
...
function setLanguage(lang) {
    ...
    // Сначала обновляем все тексты в статических элементах
    updateAllTexts();
    
    // Затем ПОЛНОСТЬЮ перерисовываем все динамические компоненты
    // Это гарантирует, что все тексты обновятся
    renderGroups();
    renderPlayoffs();
    renderResults();
    updateDrawStatus();
    
    // Дополнительно обновляем заголовки, которые могли быть перезаписаны
    setTimeout(() => {
        // Обновляем заголовки групп (они могли быть перезаписаны при renderGroups)
        const groupTitles = document.querySelectorAll('.group-header h3');
        groupTitles.forEach(title => {
            const groupLetter = title.textContent.match(/[AB]/);
            if (groupLetter) {
                title.textContent = t('group') + ' ' + groupLetter[0];
            }
        });
        
        // Обновляем заголовки таблиц в группах
        const groupTables = document.querySelectorAll('.group-teams-table th');
        if (groupTables.length >= 3) {
            groupTables[0].textContent = t('team_header');
            groupTables[1].textContent = t('wins_header');
            groupTables[2].textContent = t('points_header');
        }
        
        // Обновляем заголовки матчей групп
        const matchesHeaders = document.querySelectorAll('.group-matches-header h4');
        matchesHeaders.forEach(header => {
            const groupLetter = header.textContent.match(/[AB]/);
            if (groupLetter) {
                header.textContent = t('matches_header') + ' ' + groupLetter[0];
            }
        });
        
        // Обновляем заголовки плей-офф
        ...
        // Обновляем лейблы в нижней сетке
        const lowerLabels = document.querySelectorAll('.lower-bracket .match-label');
        if (lowerLabels[0]) lowerLabels[0].textContent = t('lower_semi');
        if (lowerLabels[1]) lowerLabels[1].textContent = t('lower_final');
        
        // Обновляем заголовок "ЧЕМПИОН:" в трофее
        const winnerLabel = document.querySelector('.winner-label');
        if (winnerLabel) winnerLabel.textContent = t('champion');
...
async function start() {
    ...
    await loadPrizes();
    await loadTeamsAndAvatarsFromSheet();
